...
export function TimelineItem({
  ...
  return (
    ...
    >
      <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-neutral-400 shadow-[0_0_10px_rgba(163,163,163,0.8)] border-2 border-white/20 z-10" />
      <div className="bg-neutral-900/20 border border-white/10 rounded-3xl p-8 backdrop-blur-xl relative overflow-hidden group hover:border-neutral-400/30 transition-colors duration-500 shadow-[0_0_40px_-10px_rgba(0,0,0,0.5)]">
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
            <div>
              <h3 className="text-xl font-heading font-semibold text-neutral-50 mb-1 group-hover:text-neutral-200 transition-colors">
                {title}
              </h3>
            ...
            <div className="flex items-center text-neutral-400 text-sm mt-2 md:mt-0 bg-white/5 px-3 py-1 rounded-full border border-white/5">
          ...
          {projects && projects.length > 0 && (
            <div className="mb-6 space-y-4">
              <h4 className="text-sm font-semibold text-neutral-300 uppercase tracking-wide">Key Projects:</h4>
              {projects.map((project, idx) => (
                <div key={idx} className="bg-black/20 border border-white/5 rounded-xl p-4">
                  <div className="flex items-start justify-between mb-2">
                    <h5 className="text-base font-medium text-neutral-200">{project.name}</h5>
                    {project.link && (
                      <a
                        href={project.link}
                        target="_blank"
